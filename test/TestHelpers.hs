...
mkTaskIn c title = mkTaskBody c title ""

-- | A headless-ready task carrying a body; returns its id.
mkTaskBody :: Connection -> Text -> Text -> TaskState -> IO Text
mkTaskBody c title body st =
    RT.insertTask
        c
        RT.NewTask
            { RT.ntTitle = title
