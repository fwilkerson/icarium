...
#
# Assumptions:
#   - ./bin/icarium exists (run: make install — or see README).
...
K_PINNED=$($I know add "Dispatched agents use ./bin/icarium" \
    --domain workflow --discipline ops --body-stdin <<'EOF'
...
K_PROMPT_LOCKSTEP=$($I know add "Prompt rendering is shared code" \
    --domain dispatch --discipline haskell --body-stdin <<'EOF'
...
K_EDGE_TYPING=$($I know add "Typed edge rules" \
    --domain storage --discipline haskell --body-stdin <<'EOF'
...
Field names in each element match the DB column names (snake_case),
...
is outside the top N, AND also delete dispatch rows marked abandoned
if any. For v0 just the log files; row retention can be a follow-up.
...
to the claude CLI yet, because we didn't know the correct flag.

Once T_CLAUDE_FLAGS (the claude-flag validation task) is done and has
left a knowledge entry documenting the actual flag, add it to the args
list in Icarium.Dispatch.runClaudeStreaming.
...
T_SIGINT=$($I task add \
    ...
    --body-stdin <<'EOF'
`icarium run` should catch SIGINT. On first Ctrl-C: finish the current
dispatch, then exit cleanly without picking a new task. On second Ctrl-C:
propagate (hard kill).

Use System.Posix.Signals.installHandler with a shared MVar flag the
loop checks between dispatches. Don't try to kill an in-flight claude
process; let it finish the task it started.

Test: start `./bin/icarium run --max 5 --dry-run`, Ctrl-C during the
...
mark the old as stale and cascade: any knowledge derived_from the old
also becomes stale (transitively).
...
Also marked `idea` — decide workflow (interactive vs scripted)
before implementing.
EOF
)
